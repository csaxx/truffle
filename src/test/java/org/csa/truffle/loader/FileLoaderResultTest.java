...
class FileLoaderResultTest {
    ...
    void loadResult_success_containsExpectedFiles() throws Exception {
        try (FileLoader loader = new FileLoader(new ResourceSource("python_hr_v1"))) {
            LoadResult result = loader.load();
            assertNotNull(result.fileContents());
            assertTrue(result.fileContents().containsKey("file_only_in_v1.py"));
            assertTrue(result.fileContents().containsKey("file_in_both_unchanged.py"));
            assertTrue(result.fileContents().containsKey("file_in_both_changed.py"));
        }
    }
    ...
    void loadResult_reloadedCallback_receivesFileContents() throws Exception {
        ...
        LoadCallback callback = new LoadCallback() {
            @Override public void reloaded(Map<String, String> fc, LoadStatus s) { received.set(fc); }
        ...
        try (FileLoader loader = new FileLoader(new ResourceSource("python_hr_v1"), callback)) {
            ...
            assertTrue(received.get().containsKey("file_only_in_v1.py"));
        }
    }

    @Test
    void loadResult_reloadedCallback_receivesStatus() throws Exception {
        AtomicReference<LoadStatus> received = new AtomicReference<>();
        LoadCallback callback = new LoadCallback() {
            @Override public void reloaded(Map<String, String> fc, LoadStatus s) { received.set(s); }
            @Override public void error(LoadStatus s, Exception e) {}
        };
        try (FileLoader loader = new FileLoader(new ResourceSource("python_hr_v1"), callback)) {
            LoadResult result = loader.load();
    ...
    void loadResult_error_fileContentsIsNull() throws Exception {
        try (FileLoader loader = new FileLoader(new FileLoaderTest.FailingSource())) {
            LoadResult result = loader.load();
            assertNull(result.fileContents());
        }
    }
    ...
    void loadResult_errorCallback_isCalled() throws Exception {
        ...
        LoadCallback callback = new LoadCallback() {
            @Override public void reloaded(Map<String, String> fc, LoadStatus s) {}
            @Override public void error(LoadStatus s, Exception e) { errorCalled.set(true); receivedEx.set(e); }
        };
    ...
    void loadResult_errorCallback_notFired_onSuccess() throws Exception {
        ...
        LoadCallback callback = new LoadCallback() {
            @Override public void reloaded(Map<String, String> fc, LoadStatus s) {}
            @Override public void error(LoadStatus s, Exception e) { errorCalled.set(true); }
        };
        try (FileLoader loader = new FileLoader(new ResourceSource("python_hr_v1"), callback)) {
            loader.load();
    ...
    void loadResult_reloadedCallback_notFired_onError() throws Exception {
        ...
        LoadCallback callback = new LoadCallback() {
            @Override public void reloaded(Map<String, String> fc, LoadStatus s) { reloadedCalled.set(true); }
            @Override public void error(LoadStatus s, Exception e) {}
        };
        try (FileLoader loader = new FileLoader(new FileLoaderTest.FailingSource(), callback)) {
            loader.load();
